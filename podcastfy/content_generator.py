...
class LLMBackend:
	def __init__(self, is_local: bool, temperature: float, max_output_tokens: int, model_name: str):
		"""
...
class ContentGenerator:
	...
	def generate_qa_content(self, input_texts: str = "", image_file_paths: List[str] = [], output_filepath: Optional[str] = None, is_local: bool = False) -> str:
		"""
		...
		try:
			llmbackend = LLMBackend(
				...
				model_name=self.content_generator_config.get('gemini_model', 'gemini-1.5-pro-latest') if not is_local else "User provided model"
			)
