...
class LLMBackendConfig:
    ...
    def __init__(
        ...
        """
        self.type = type
        self.is_local = self.type == LLMBackendType.OLLAMA
        self.temperature = temperature
        self.max_output_tokens = max_output_tokens
        self.model_name = model_name
        self.api_key_label = api_key_label
        logger.debug(f"LLMBackendConfig: {self.__dict__}")


class LLMBackend:
    def __init__(self, llm_config: LLMBackendConfig):
        """
        Initialize the LLMBackend with the specified configuration.
        ...
        logger.debug(f"Initializing LLM backend: {self.config.type}")

        if self.config.type == "ollama":
            logger.debug(f"Initializing OllamaLLM with model {self.config.model_name}")
            self.llm = OllamaLLM(
            ...
            logger.debug(f"Initializing ChatGoogleGenerativeAI with model {self.config.model_name}")
            ...
            logger.debug(f"Initializing ChatLiteLLM for type {self.config.type} with model {self.config.model_name}")
            self.llm = ChatLiteLLM(
                model=self.config.model_name,
                temperature=self.config.temperature,
                api_key=os.environ[self.config.api_key_label],
            )
...
class LongFormContentStrategy(ContentGenerationStrategy, ContentCleanerMixin):
    ...
    def _clean_transcript_response_DEPRECATED(
        self, transcript: str, config: Dict[str, Any]
    ) -> str:
        """
        Clean transcript using a two-step process with LLM-based cleaning.
        ...
        try:
            ...
            try:
                clean_transcript_prompt = hub.pull(
                    f"{self.content_generator_config['cleaner_prompt_template']}:{self.content_generator_config['cleaner_prompt_commit']}"
                )
                rewrite_prompt = hub.pull(
                    f"{self.content_generator_config['rewriter_prompt_template']}:{self.content_generator_config['rewriter_prompt_commit']}"
                )
...
class ContentGenerator:
    def __init__(
        self,
        backend_type: LLMBackendType = LLMBackendType.GOOGLE,
        model_name: str|None = "gemini-1.5-pro-latest",
        ...
        llm_backend = LLMBackend(
            LLMBackendConfig(
                backend_type,
                self.config_conversation.get("creativity", 1),
                self.content_generator_config.get("max_output_tokens", 8192),
                model_name or "gemini-1.5-pro-latest",
                api_key_label or "GEMINI_API_KEY",
            )
        )
